...
namespace CarRentalSystem.Controller.Logistics;

[ApiController]
[Route("api/[controller]")]
[Authorize(Roles = "Admin,Agent")] // Gate operations are Agent / Admin only
public class GateLogisticsController : ControllerBase
{
    ...
    // Agent / Admin — process vehicle checkout (car leaves the lot)
    [HttpPost("checkout/{reservationId}")]
    public async Task<IActionResult> ChecklistCheckout(int reservationId,
                                                        [FromBody] GateCheckoutRequest request)
    {
    ...
    // Agent / Admin — process vehicle check-in (car returns to the lot)
    [HttpPost("checkin/{reservationId}")]
    public async Task<IActionResult> ChecklistCheckin(int reservationId,
                                                       [FromBody] GateCheckinRequest request)
    {
        if (!ModelState.IsValid) return BadRequest(ModelState);
    ...
    // Agent / Admin — retrieve checkout + checkin log for a reservation
